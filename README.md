# LLM 全栈工程 · LLM 工业级全栈工程落地手册

> 面向工业落地的 LLM 全栈工程手册（VoiceAI前沿 出品）：不是学术讲义，而是**能照做的产线手册**。
> 第 00 章架构通识 + 主线 46 章 = 47 篇；每篇独立可读，且**每章都配有可直接运行的工程代码**（共 84 个脚本，含真实运行输出与一键启动命令）。

**这份仓库解决什么问题？**

| 你会遇到的工业问题 | 对应内容 | 对应代码 |
|---|---|---|
| 语料脏、重复、格式乱、质量不可控 | 第 04-10 章 | code/data_engineering/ |
| 训练中断白跑、不收敛、过拟合、灾难遗忘 | 第 11-13/18-19 章 | code/training_tools/ |
| SFT/DPO/RLHF 效果差、对齐不稳 | 第 14-26 章 | code/minimind_style + training_tools/ |
| 推理慢、显存爆、吞吐低、流式卡顿 | 第 27-34 章 | code/deployment/ |
| 编译器/投机/批调度等极致优化怎么落地 | 第 35-40 章 | code/deployment/ |
| 上线后怎么迭代、对标、排查 | 第 41-45 章 | code/ops_pipeline/ |

## 手册内容结构（按工业产线划分）

| 范围 | 篇目 | 覆盖内容 |
|---|---|---|
| 第 00 章 · 架构通识 | 00–00 | RNN/LSTM/GRU、Transformer、Mamba/SSM、MoE、稀疏注意力、世界模型、音频/视觉/多模态架构与横向对比 |
| 第 0 阶段 · 开篇引路 | 01–03 | 全栈地图、2 小时最小闭环、预训练原理 |
| 第 1 阶段 · 预训练工程·数据核心层 | 04–13 | 数据管道、清洗、过滤、去重、脱敏、质量打分、领域数据、增量预训练、超参与训练稳定性 |
| 第 2 阶段 · 微调与对齐体系 | 14–26 | SFT、数据标注、模板、调参、拟合与遗忘、评估、多轮、蒸馏、RM、PPO、RLAIF、冷启动 |
| 第 3 阶段 · 基础部署与推理优化 | 27–34 | 框架对比、vLLM、SGLang、TensorRT-LLM、量化、剪枝、KV Cache、流式与高并发 |
| 第 4 阶段 · 高阶推理编译器极致优化 | 35–40 | 编译器原理、算子融合、动态 shape、投机解码、批量并行、内核与编译级量化 |
| 第 5 阶段 · 全栈工程联调与项目落地 | 41–46 | 全链路、领域定制、高并发与端侧、模型迭代、线上排障、最佳实践 |

## 目录结构

```text
llm-fullstack-handbook/
├── README.md            # 本文件：仓库总览
├── md/                  # 47 篇 Markdown 源稿（第 00 章 + 主线 46 章）
├── code/                # 可运行工程代码（47 章全覆盖，见下方导航）
│   ├── 00_* ~ 06_*      # 第 00 章模型结构代码（RNN/Transformer/Mamba/MoE/稀疏注意力/多模态/世界模型）
│   ├── minimind_style/  # MiniMind 式微型 LLM 全链路（预训练/SFT/LoRA/DPO/MoE/GRPO/变体模型）
│   ├── data_engineering/# 主线 04-10：数据管道/清洗/过滤/去重/脱敏/质量/领域构建
│   ├── training_tools/  # 主线 11-26：增量续训/超参/过拟合/评估/多轮/蒸馏/RM/PPO/RLAIF/领域自适应
│   ├── deployment/      # 主线 27-40：框架/量化/剪枝/流式/编译器/投机解码/批调度
│   ├── ops_pipeline/    # 主线 41-45：端到端/ONNX/AB 对标/监控
│   ├── CHAPTER_CODE_MAP.md   # 47 章 ↔ 代码映射表
│   └── CODE_STANDARD.md      # 全局代码落地规范与验收清单
├── docs/                # GitHub Pages 站点（发布根目录）
│   ├── index.html       # 首页：阶段章节总目录
│   ├── chapters/        # ch00.html … ch46.html
│   ├── assets/style.css
│   └── .nojekyll
└── scripts/
    └── build_site.py    # 由 md/ 重新生成 docs/ 的构建脚本
```

## 代码仓库导航（47 章全覆盖）

每个章节都有对应的可一键运行工程代码；全部文件遵守 `code/CODE_STANDARD.md`（9 个文档字段 + L1/L2/L3 三层标注），运行示例均为真实输出。

| 目录 | 覆盖章节 | 内容 | 入口 |
|---|---|---|---|
| `00_* ~ 06_*` | 第 00 章 | 模型结构原理可运行代码（每个架构目录三层） | `code/README.md` |
| `minimind_style` | 01-03 主线 + 模型变体 | 从 0 训练微型 LLM 全流程 | `code/minimind_style/README.md` |
| `data_engineering` | 04-10 | 数据管道/清洗/过滤/去重/脱敏/质量/领域构建 | 各文件头部说明 |
| `training_tools` | 11-26 | 训练与对齐工具链 | 各文件头部说明 |
| `deployment` | 27-40 | 部署/压缩/推理优化（含 dry-run 启动器） | 各文件头部说明 |
| `ops_pipeline` | 41-45 | 端到端/领域启动/ONNX/AB/监控 | 各文件头部说明 |

- 47 章 ↔ 代码对照总表：`code/CHAPTER_CODE_MAP.md`
- 全局代码规范与合规检查器：`code/CODE_STANDARD.md` + `code/scripts/check_code_standard.py`

### 三分钟跑通最小闭环（CPU）

```bash
cd code/minimind_style
pip install torch==2.2.2
python pretrain.py --steps 120      # 微型预训练
python train_sft.py --epochs 10     # 微型 SFT
python generate.py --prompt "人工智能"
```

## 本地预览

```bash
cd docs
python3 -m http.server 8000
# 浏览器打开 http://127.0.0.1:8000
```

## 重新生成站点

```bash
python3 scripts/build_site.py
```

## 发布到 GitHub Pages

1. 仓库 Settings → Pages → Build and deployment → Source 选择「Deploy from a branch」；
2. Branch 选 `main`，目录选 `/docs`；
3. 站点地址：https://fenglinxin.github.io/llm-fullstack-handbook/

<details>
<summary>📚 章节索引（47 篇，点击展开）</summary>

## 章节索引（47 篇）


- 第 00 章 · 架构通识
  - 【LLM全栈工程·第00章】模型结构核心原理层：主流架构一张图讲透（RNN→Transformer→Mamba→MoE→多模态）

- 第 0 阶段 · 开篇引路
  - 【LLM全栈工程·第01章】一条大模型生产线全程发生了什么？0 基础也能看懂的 LLM 全栈地图与学习路线
  - 【LLM全栈工程·第02章】2 小时跑通最小闭环：从开源模型下载到 LoRA 微调再到推理服务
  - 【LLM全栈工程·第03章】预训练到底在练什么？整体架构、训练逻辑与关键概念

- 第 1 阶段 · 预训练工程·数据核心层
  - 【LLM全栈工程·第04章】数据管道从零构建：原始采集、多源整合与目录设计
  - 【LLM全栈工程·第05章】数据清洗规范与脏数据剔除：先认识“脏”长什么样
  - 【LLM全栈工程·第06章】文本过滤实战：低质、重复、有害内容的过滤体系
  - 【LLM全栈工程·第07章】数据去重算法：从精确去重到 MinHash 近似去重
  - 【LLM全栈工程·第08章】敏感数据脱敏：PII 识别、脱敏策略与合规落地
  - 【LLM全栈工程·第09章】数据质量打分体系：规则、分类器与多维度融合
  - 【LLM全栈工程·第10章】领域专属数据构建：采集、清洗与合成数据
  - 【LLM全栈工程·第11章】增量预训练方案：领域适配、学习率与灾难遗忘
  - 【LLM全栈工程·第12章】预训练超参选型与硬件适配：把每一分算力花在刀刃上
  - 【LLM全栈工程·第13章】断点续训、收敛判断与训练失败排查：把“训练翻车”变成流程

- 第 2 阶段 · 微调与对齐体系
  - 【LLM全栈工程·第14章】全维度微调技术拆解：全参/LoRA/QLoRA/适配器怎么选
  - 【LLM全栈工程·第15章】SFT 监督微调实战：数据、训练、评估到合并上线
  - 【LLM全栈工程·第16章】SFT 数据集构建与标注规范：指令设计、标注与质控
  - 【LLM全栈工程·第17章】微调模板设计：Chat Template、角色设定与多轮拼接
  - 【LLM全栈工程·第18章】微调超参调优策略：rank/alpha/lr/epoch 与实验管理
  - 【LLM全栈工程·第19章】过拟合、欠拟合与灾难遗忘：症状、归因与解法
  - 【LLM全栈工程·第20章】微调效果评估体系：任务集、基准、消融与 LLM 裁判
  - 【LLM全栈工程·第21章】多轮对话微调专项：长上下文、多轮数据与稳定性
  - 【LLM全栈工程·第22章】模型蒸馏实战：大模型萃取小模型的完整流程
  - 【LLM全栈工程·第23章】奖励模型训练：偏好数据、训练与防 Reward Hacking
  - 【LLM全栈工程·第24章】RLHF/PPO 工程落地：参考模型、KL、PPO 与资源规划
  - 【LLM全栈工程·第25章】RLAIF：AI 反馈自动对齐与偏好合成
  - 【LLM全栈工程·第26章】小样本与领域自适应微调：冷启动方案

- 第 3 阶段 · 基础部署与推理优化
  - 【LLM全栈工程·第27章】部署框架横向对比：vLLM/SGLang/TensorRT-LLM/llama.cpp
  - 【LLM全栈工程·第28章】vLLM 部署实战与参数调优：从启动到生产级
  - 【LLM全栈工程·第29章】SGLang 高性能推理落地：前缀复用与结构化输出
  - 【LLM全栈工程·第30章】TensorRT-LLM 固化与加速：从权重到 Engine
  - 【LLM全栈工程·第31章】模型量化实操：FP16/BF16/INT8/INT4 怎么选怎么用
  - 【LLM全栈工程·第32章】模型剪枝与蒸馏压缩：小模型化落地
  - 【LLM全栈工程·第33章】KV Cache 优化与上下文窗口拓展：长上下文的成本与解法
  - 【LLM全栈工程·第34章】流式推理封装与高并发服务化：从单实例到生产网关

- 第 4 阶段 · 高阶推理编译器极致优化
  - 【LLM全栈工程·第35章】推理编译器核心原理：图编译、IR、调度与代码生成
  - 【LLM全栈工程·第36章】算子融合与计算图编译实战：把零散 kernel 合成一个
  - 【LLM全栈工程·第37章】动态 shape 适配与显存编译器优化：让编译不怕“变”
  - 【LLM全栈工程·第38章】Speculative Decoding 投机推理进阶：让大模型“先猜后验”
  - 【LLM全栈工程·第39章】批量调度与并行深度适配：continuous batching、PD 分离与 TP/PP/EP
  - 【LLM全栈工程·第40章】内核重构与编译级量化：硬件指令集与极致优化总纲

- 第 5 阶段 · 全栈工程联调与项目落地
  - 【LLM全栈工程·第41章】全链路串联：数据→训练→对齐→部署的一体化流程
  - 【LLM全栈工程·第42章】领域大模型定制化落地：从立项到上线
  - 【LLM全栈工程·第43章】高并发生产适配与端侧部署：GPU 池、弹性与端云协同
  - 【LLM全栈工程·第44章】模型迭代升级与性能对标评测：让模型持续变好且可证明
  - 【LLM全栈工程·第45章】线上问题闭环排查：监控、告警与应急手册
  - 【LLM全栈工程·第46章】工程化最佳实践汇总：LLM 工业级全栈落地手册（终章）

</details>

## 说明

- 署名：VoiceAI前沿（原“Voice前沿”）
- 定位：工业落地导向。正文与代码按“照做即可跑通”编写，所有运行示例为真实输出；
  框架类部署脚本（vLLM/SGLang/TensorRT-LLM）提供参数校验与 dry-run，`--run` 请按官方文档在对应环境执行
- 稿件最初面向微信公众号连载，本仓库提供 Markdown 源稿与通用阅读站点；
- 文中含部署命令的教学骨架：涉及版本、模型名、端口等请以各项目官方文档为准。