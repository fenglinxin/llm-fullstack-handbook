...
【层级】L2（工程训练：扩散语言模型训练+完形演示）
【核心逻辑】语料滑窗随机涂黑（mask_prob+span_prob 混合），只还原被涂位置；
val 用语料尾部未见窗口测泛化；完形演示取训练域内句子验证双向信息。
【运行结果示例】（真实运行，CPU，3000 步 dim64）
step 2999 loss 0.0462 val_acc 0.136 | 完形: 监督微调让模型学会按照指令回答[问题]。
训练窗口 span 还原率 98.7%（val 是未见尾部文本，低分=没作弊）
【高频报错 Top5】
1. val_acc 低是设计如此（val 取未见文本），别当 bug；
2. 完形错字：span 训练不够/容量小；修复：加大 --steps/--dim；
3. infill 位置错位：off 偏移在窗口截断时算错（已处理，改动后跑自检）；
4. mask 全 True/全 False：mask_prob 极端值；修复：0.15-0.5；
5. 想自由续写：扩散模型不适合，换 generate 但预期质量差。
【工程改造方向】接真实语料；还原调度；批量并行还原提速。
"""
